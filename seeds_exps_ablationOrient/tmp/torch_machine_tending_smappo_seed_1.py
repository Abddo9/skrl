...
class Policy(GaussianMixin, Model):
    def __init__(self, observation_space, action_space, device, clip_actions=False,
        ...
        self.agent_enc = nn.Sequential(nn.Linear(self.agent_feature_size+self.lidar_embed_dim-1, self.atten_embed_dim), nn.ReLU(), nn.LayerNorm(self.atten_embed_dim))
        self.machines_enc = nn.Sequential(nn.Linear(self.machine_feature_size, self.atten_embed_dim), nn.ReLU(), nn.LayerNorm(self.atten_embed_dim))
        self.storages_enc = nn.Sequential(nn.Linear(self.storage_feature_size, self.atten_embed_dim), nn.ReLU(), nn.LayerNorm(self.atten_embed_dim))
        self.other_agents_enc = nn.Sequential(nn.Linear(self.agent_feature_size-1, self.atten_embed_dim), nn.ReLU(), nn.LayerNorm(self.atten_embed_dim))
    ...
    def encode_objects(self, obs):
        ...
        agent_info = agent_info[:, :, [0,1,3,4,5]]  # Remove orientation
        other_agents_info = other_agents_info[:, :, [0,1,3,4,5]]  # Remove orientation
